...
class ApiClient {
  ...
  async manualRegisterGroup(data: {
    bot_id: string;
    group_id: string;
    group_name?: string;
    group_type?: string;
  ...
  async translateAndSaveUserAgreement(text: string) {
    const response = await this.client.post(
      '/admin/system-settings/user-agreement/translate-and-save',
      { text }
    );
    return response.data;
  }
  ...
  async resetCustomPrice(pairId: string, price: number) {
    const response = await this.client.post(`/admin/trading/pairs/${pairId}/reset-price`, {
      price,
    });
    return response.data;
  }
  ...
  async clearNetworkDerivedAddresses(networkId: string | 'all') {
    const response = await this.client.delete(
      `/admin/wallet/networks/${networkId}/derived-addresses`
    );
    return response.data;
  }
  ...
  async setupNetworkStream(
    id: string,
    data: { moralis_api_key?: string; trongrid_api_key?: string; webhook_url: string }
  ) {
  ...
  async adjustBalance(
    userId: string,
    data: { amount: number; type: 'add' | 'subtract'; reason?: string }
  ) {
  ...
  async grantInviteReward(userId: string, inviteeId: string) {
    const response = await this.client.post(
      `/users/${userId}/invitees/${inviteeId}/grant-reward`
    );
    return response.data;
  }
  ...
  async ignoreInviteReward(userId: string, inviteeId: string) {
    const response = await this.client.post(
      `/users/${userId}/invitees/${inviteeId}/ignore-reward`
    );
    return response.data;
  }
  ...
  async getTodayResults(pairId: string | number) {
    const response = await this.client.get('/admin/trading/sessions/today-results', {
  ...
  async uploadLandingLogo(file: File): Promise<{ logoUrl: string }> {
    ...
    const response = await this.client.post<{ logoUrl: string }>(
      '/admin/landing/upload-logo',
      formData,
      { headers: { 'Content-Type': 'multipart/form-data' } }
    );
    return response.data;
  ...
  async uploadInviteCardImage(file: File): Promise<{ url: string }> {
    ...
    const response = await this.client.post<{ url: string }>(
      '/admin/system-settings/invite-card/upload',
      formData,
      { headers: { 'Content-Type': 'multipart/form-data' } }
    );
    return response.data;
  ...
  async uploadMiniAppBgImage(file: File): Promise<{ url: string }> {
    ...
    formData.append('image', file);
    const response = await this.client.post<{ url: string }>(
      '/admin/system-settings/miniapp-bg/upload',
      formData,
      { headers: { 'Content-Type': 'multipart/form-data' } }
    );
    return response.data;
  ...
  async translateAndSaveInviteMessage(
    text: string
  ): Promise<{ translations: Record<string, string>; saved_keys: string[] }> {
    const response = await this.client.post(
      '/admin/system-settings/invite-message/translate-and-save',
      { text }
    );
    return response.data;
  }

  async saveInviteReward(data: {
    amount?: number;
    enabled?: boolean;
