...
/**
 * DELETE /api/charity/projects/:id
 * Permanently delete a charity project (admin)
 */
router.delete('/projects/:id', adminLimiter, authenticateAdmin, async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
    const result = await query(`DELETE FROM charity_projects WHERE id = $1 RETURNING id`, [id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Project not found' });
    }
