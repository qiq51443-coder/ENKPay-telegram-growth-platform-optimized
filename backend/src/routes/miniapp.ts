 ...
 * Called by the Bot to store a one-time jt token in Redis.
 ...
 * Exchange a one-time ?jt= URL token (generated by the Bot on /start) for a
 * session token + canonical user profile.
 * No Telegram initData or HMAC required — the token itself is the credential.
 * The token is single-use: consumed atomically on first call.
 */
