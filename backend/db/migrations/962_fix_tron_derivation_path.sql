...
-- TronLink and other standard TRON wallets.
--
-- Fix:
--   1. Update hd_derivation_path to m/44'/195'/0'/0 for all TRON networks
--   2. Delete stale hd_derived addresses so they are regenerated correctly
--   3. Also ensure chain_name case is consistent (TRON uppercase)
...
-- They will be re-derived correctly on the next user request using the fixed path
