...
const MAX_POLL_ATTEMPTS = 50; // 5 seconds total — aligned with jt-auth Phase 1 network timeout
    ...
    const timer = setInterval(() => {
      ...
      if (attempts >= MAX_POLL_ATTEMPTS) {
        ...
        if (webApp && webApp.initData === '') {
          console.warn('[useTelegram] Telegram SDK present but no initData after 5s. Not opened from Telegram?');
        }
